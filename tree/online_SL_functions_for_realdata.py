...
def information_distance_online(data, EX_inform_online, EX2_inform_online, lam=0.40, tol=0.0001):
    """
    基于在线学习计算信息距离矩阵：-log(|correlation|)。

    输入:
      data: 当前批次的数据，二维 NumPy 数组，行表示样本，列表示变量
      EX_accum: 之前累计样本一阶矩，用于增量计算（初始化为 None）
      EX2_accum: 之前累计样本二阶矩（初始化为 None）
      sample_count: 当前已处理的样本数，用于增量计算（初始化为 0）
      tol: 判断变量是否为离散的阈值（默认 1e-4）

    输出:
      result: pandas DataFrame，信息距离矩阵，每个元素为 -log(|相关系数|)。
    """
    # 初始化EX和EX2
    if np.all(EX_inform_online == 0):
        EX_inform_online = np.zeros(data.shape[1])
    if np.all(EX2_inform_online == 0):
        EX2_inform_online = np.zeros((data.shape[1], data.shape[1]))

    # print("EX_inform_online1-1:", EX_inform_online)
    # print("EX2_inform_online1-1:", EX2_inform_online)

    # 基于当前时刻数据的样本一阶矩和二阶矩
    EX_new = np.mean(data, axis=0)
    EX2_new = (data.T @ data) / data.shape[0]

    print('EX_new前', EX_new)
    print('EX2_new前', EX2_new)
    # online
    # print("lam:", lam)
     # 1) 缩放旧值
    EX_inform_online  *= lam
    EX2_inform_online *= lam
    # print('EX_inform_online缩放后', EX_inform_online)
    # print('EX2_inform_online缩放后', EX2_inform_online)
    # 2) 累加新值
    EX_inform_online  += (1 - lam) * EX_new
    EX2_inform_online += (1 - lam) * EX2_new
    # print('EX_inform_online最后', EX_inform_online)
    # print('EX2_inform_online最后', EX2_inform_online)
    # 计算协方差矩阵 Sigma2 = EX2 - EX*EX^T
    Sigma2 = EX2_inform_online - np.outer(EX_inform_online, EX_inform_online)
    print(f"Sigma2:{Sigma2}")

    # 构造对角矩阵：1/sqrt(diag(Sigma2))，避免除零错误
    diag_vals = np.diag(Sigma2)
    print(f"diag_vals:{diag_vals}")
    inv_sqrt = 1.0 / np.sqrt(diag_vals + tol)      # (D,)
    print(f"inv_sqrt:{inv_sqrt}")
    # 直接对协方差矩阵做广播缩放
    rho = Sigma2 * inv_sqrt[:, None] * inv_sqrt[None, :]
    ...
    return result, EX_inform_online, EX2_inform_online


def compute_true_moments_from_para(model, para):
    """
    给定 model 和 对应时刻的真实参数 para，
    精确计算每个观测变量的总体均值 E[X] 和二阶原点矩 E[X^2]。

    返回:
        means:   dict {node_name -> E[X]}
        seconds: dict {node_name -> E[X^2]}
    """
    # 观测节点：连续 + 离散
    ...
    print(f"obs_nodes:{obs_nodes}")
    # 隐节点
    lat_mask = model["vertice_type"] == "latent"
    print(f"lat_mask:{lat_mask}")
    lat_nodes = model.loc[lat_mask, "vertice"].tolist()
    print(f"lat_nodes:{lat_nodes}")
    # 父节点字典
    father = model.set_index("vertice")["father_vertice"].to_dict()
    print(f"father:{father}")
    # 结果容器
    means = {v: 0.0 for v in obs_nodes}
    seconds = {v: 0.0 for v in obs_nodes}

    L = len(lat_nodes)
    if L == 0:
        raise ValueError("模型中没有隐节点，当前函数假设存在二元隐变量。")

    # 辅助函数：给定隐变量配置 assignment，计算 P(z)
    def latent_joint_prob(assignment):
        prob = 1.0
        for h in lat_nodes:
            pa_h = father.get(h, "")
            val_h = assignment[h]

            if pa_h == "" or pa_h is None:
                # 根 latent: para[h] 为长度 1 的数组 [P(h=1)]
                p_arr = np.array(para[h]).reshape(-1)
                p1 = float(p_arr[0])
                prob *= p1 if val_h == 1 else (1.0 - p1)
            else:
                # 有父节点的 latent: para[h] 形状 (2,1)
                # 第 0 行：P(h=1 | parent=0)，第 1 行：P(h=1 | parent=1)
                p_mat = np.array(para[h]).reshape(2, -1)
                parent_state = assignment[pa_h]
                p1 = float(p_mat[parent_state, 0])
                prob *= p1 if val_h == 1 else (1.0 - p1)
        return prob
    ...
    for bits in range(2 ** L):
        assignment = {}
        for idx, h in enumerate(lat_nodes):
            assignment[h] = (bits >> idx) & 1
        print(f"assignment:{assignment}")
        p_z = latent_joint_prob(assignment)
        if p_z == 0.0:
            continue

        # 对每个观测变量，在该隐状态下算条件矩，再用 p_z 加权
        for v in obs_nodes:
            v_type = model.loc[v, "vertice_type"]
            pa_v = father[v]

            if pa_v == "" or pa_v is None:
                raise ValueError(f"观测节点 {v} 没有父节点，当前模型假设观测都有 latent 父节点。")

            parent_state = assignment[pa_v]

            if v_type == "observable_discrete":
                # para[v] 形状 (2,1)：P(X=1|parent=0/1)
                p_mat = np.array(para[v]).reshape(2, -1)
                p1 = float(p_mat[parent_state, 0])
                m = p1
                m2 = p1  # Bernoulli: E[X^2]=E[X]
            elif v_type == "observable_continuous":
                # para[v] 形状 (4,1)：[mu0, mu1, sigma0, sigma1]
                p4 = np.array(para[v]).reshape(4, -1)[:, 0]
                if parent_state == 0:
                    mu = float(p4[0])
                    sigma = float(p4[2])
                else:
                    ...
                    sigma = float(p4[3])
                m = mu
                m2 = mu * mu + sigma * sigma
            else:
                raise ValueError(f"未知的 vertice_type: {v_type}")

            means[v] += p_z * m
            seconds[v] += p_z * m2

    # 简单检查概率是否接近 1（可选）
    # total_p = sum(latent_joint_prob({h: (bits >> idx) & 1 for idx, h in enumerate(lat_nodes)})
    #               for bits in range(2 ** L))
    # print("Sum of P(z) over all latent configs: ", total_p)

    return means, seconds

def information_distance_online_no(data, EX_inform_online, EX2_inform_online, tol=0.0001):
    """
    基于在线学习计算信息距离矩阵：-log(|correlation|)。
...
def update_tree(sab_groups, tree):
    ...
    for i, new_node in enumerate(lat_nodes_new):
        # 先将 group 中的索引转换为字符串
        children_list = [str(node + 1) if isinstance(node, int) and node < len(tree["obs_nodes"]) else str(node) for
                         node in sab_groups[i]]
...
def SLLT(D, tau1=3, tau2=5, epson_init=0.1, step=0.1, epson2=0.1):
    """
