...
def para_esti(i, Aijk, mu_k, node_type="discrete", mu_i=None, mu_i2=None, 
    ...
    if node_type == "discrete":
        ...
        lam=3 #lam越大两个均值越趋于相同，lam可以用来控制两个均值的分离程度，另外，关于beta2的平方不如绝对值的效果好
...
def GotA(ijk, data, tol=0.0001):
    ...
    if i_discrete and j_discrete:
        ...
        aj2 = data[:, j]
        # 保存 mu_i 与 mu_i2
        mu_i_dict["mu_i"] = np.mean(ai)
        mu_i_dict["mu_i2"] = np.mean(ai2)
...
def para_esti_obs(ijk, h=None, flag=None, tree=None, data=None, tol=1e-6, 
    ...
    if para is None or model is None or prob_marg is None:#------------输入的是数据
        ...
        if data is None:
            raise ValueError("Please input complete parameters 'model', 'para' and 'para_marg' or the parameter 'data'!")
        ...
        for l in range(len(I)):
            ...
            para_input[i] = GotA(ijk[l], data, tol)
    else:#-------------------------------------------------------------输入的是真实的模型参数
        for l in range(len(I)):
            i = I[l]
    ...
    if len(V) >= 2:#-------------------------------------------------如果h1有超过2个可观测子节点
        ...
        if len(H) >= 3:#-----------------------------如果h1有多于3个隐子节点
            ...
            if pa[h1] == "":#-----------------------------如果h1是根节点
                ...
                df = pd.DataFrame({h_u: ijk_dict[h_u], "root": [I_H[h_u], I_H_list[1], I_H_list[0]]}, index=["1", "2", "3"])
...
def solve_nnls_2x2(temp_ch, temp_pa):
    ...
    try:
        # 使用伪逆求解初始值
        beta = np.linalg.pinv(A_reg) @ b_reg
    ...
    try:
        # 使用修改后的正则化方法
        lam = 1e-1  # 减小正则化参数
        diag_reg = np.diag(lam / (np.abs(beta) + eps))
        A_final = A_reg + diag_reg
        
        # 使用 scipy.linalg.solve 代替 lsq_linear
        from scipy.linalg import solve
        try:
            x = solve(A_final, b_reg, assume_a='sym')
            # 确保非负
            x = np.maximum(x, 1e-3)
        except:
            # 如果直接求解失败，使用 SVD 分解
            U, s, Vh = np.linalg.svd(A_final)
            s[s < eps] = eps  # 避免奇异值太小
            x = Vh.T @ (np.diag(1/s) @ (U.T @ b_reg))
            x = np.maximum(x, 1e-3)
    except:
        # 最后的备选方案：使用简单的非负最小二乘
...
#输出: {I[1]:{...}; I[2]:{...}; I[3]:{...}; ...}
def para_esti_lat(ijk, h=None, flag=None, para_hat=None, tree=None, data=None, tol=1e-6, 
                  model=None, para=None, prob_marg=None, g=None):
    """
    ...
    if (para is None) or (model is None) or (prob_marg is None):#------------输入的是数据
        ...
        for h_u in child_latent:
            ...
            para_input_ch[i_val] = GotA(ijk=ijk[h_u].iloc[:,0] if isinstance(ijk[h_u], pd.DataFrame) else ijk[h_u][:,0],
                                         data=data, tol=tol)
            para_input_pa[i_val] = GotA(ijk=ijk[h_u].iloc[:,1] if isinstance(ijk[h_u], pd.DataFrame) else ijk[h_u][:,1],
                                         data=data, tol=tol)
        ...
        for h_u in child_latent:
            ...
            #得到子节点和父节点的三叉变量
            para_input_ch[i_val] = GotA_true(ijk=ijk[h_u].iloc[:,0] if isinstance(ijk[h_u], pd.DataFrame) else ijk[h_u][:,0],
                                             h=h_u, model=model, para=para, prob_marg=prob_marg, g=g, tree=tree)
            para_input_pa[i_val] = GotA_true(ijk=ijk[h_u].iloc[:,1] if isinstance(ijk[h_u], pd.DataFrame) else ijk[h_u][:,1],
    ...
    if sum(logi0) != 0:# 如果存在 flag==0 的情况，优先处理
        ...
        for h_u, f in flag.items():
            if f == 0:            
                ...
                if isinstance(Aijk_i_ch, list) and isinstance(Aijk_i_pa, list):#-------------------------------如果Aijk_i中有不止一个
                    print("如果Aijk_i中有不止一个")
                    p_val = max(len(Aijk_i_ch), len(mu_k_ch), len(Aijk_i_pa), len(mu_k_pa))
                    # 如果长度不一致，则报错
                    if len(Aijk_i_ch) != p_val or len(mu_k_ch) != p_val or len(Aijk_i_pa) != p_val or len(mu_k_pa) != p_val:
                        raise ValueError("'Aijk_i', 'mu_k' and 'mu_i' have different length!")
                    temp_ch = [None] * p_val
                    temp_pa = [None] * p_val
                    temp_lat = [None] * p_val
    ...
    if sum(logi1) != 0:
        ...
        for h_u, f in flag.items():#----------------------------如果存在 flag=1 的情况(只有极少数情况下才不存在)
            if f==1:
                ...
                if isinstance(Aijk_i_ch, list) and isinstance(Aijk_i_pa, list):#-------------------------------如果Aijk_i中有不止一个
                    p_val = max(len(Aijk_i_ch), len(mu_k_ch), len(Aijk_i_pa), len(mu_k_pa))
                    if not (len(Aijk_i_ch)==p_val and len(mu_k_ch)==p_val and len(Aijk_i_pa)==p_val and len(mu_k_pa)==p_val):
                        raise ValueError("'Aijk_i', 'mu_k' and 'mu_i' have different length!")
                    temp_ch = [None]*p_val
                    temp_pa = [None]*p_val
                    temp_lat = [None]*p_val
                        ...
                        if pa.get(k2, None) == h:
                            ...
                            if k2 not in I:#-----------------------pa[k2] == h时，k2不一定在I中
                                ...
                                if model is not None:
                                    ...
                                    if abs(data[0, int(k2)-1] - int(data[0,int(k2)-1])) > tol:#这里需要注意data的行与列假定是0-based，所以顶点的索引值-1
                                        lambda_val = temp_mat[0, :]
                                    else:
                                        lambda_val = temp_mat[1, :]
                                else:
...
def PEMLT(data_obs, model, tol=0.0001):
    """
    根据观测数据和模型设定进行参数估计。
    ...
    logi = [abs(data_obs[0, u] - int(data_obs[0, u])) > tol for u in range(num_cols)]#-------------------------T: 连续; F: 离散
    ...
    for h in reversed(step):#---------从底层到高层
        ...
        #     continue 
        # —— 修正 ch_obs_num，确保它是一个整数 ——  
        if h in st.index:
        # 用 at 拿单元格，避免返回 Series
            raw = st.at[h, st.columns[3]]
        ...
        para_hat_ = para_esti_obs(ijk=Ijk[h], h=h, flag=flag[h], tree=tree, data=data_obs, g=g)
    ...
    for h in reversed(step):#---------从底层到高层
        ...
        if len(ch_lat) == 0:#------------------如果h没有隐子节点，则跳过
            continue
        temp = tri_nodes_lat(h1=h, key_node=key_node, tree=tree)#return {"tri_nodes": ijkjk, "key_node": key_node, "key_node_type": flag}
        ...
        para_hat_ = para_esti_lat(ijk=Ijk[h], h=h, flag=flag[h], para_hat=para_hat, tree=tree,
                                   data=data_obs, g=g)
...
#       - 根 latent 编号为 H1，依次向下做 BFS 编号,这样对于隐变量来说，每个隐节点的父节点的编号比子节点小，H1是根节点
...
def renumber_latents_df(model: pd.DataFrame) -> pd.DataFrame:
    ...
    对 model 中 vertice_type=="latent" 的节点做层次重编号，并返回新的 DataFrame：
      ...
      - 更新 vertice 列和 father_vertice 列中的隐变量名称
      - 保留 observable 节点的原名和原顺序
      - 最终输出 DataFrame：先列出可观测节点（observable），按 vertice 数值升序；再列出隐变量（latent），按 H 后数字降序
    """
    # 1) 建立父→子索引
    children = {}
    for _, row in model.iterrows():
    ...
    # 6) 分别提取 observable 和 latent 并排序
    obs_df = new_model[new_model['vertice_type'] != 'latent'].copy()
    # 可观测节点按 vertice 数值升序
    ...
    lat_df = new_model[new_model['vertice_type'] == 'latent'].copy()
    # 隐变量按 H 后数字降序
    lat_df = lat_df.sort_values(
        by='vertice',
        key=lambda s: s.str.lstrip('H').astype(int),
        ascending=False
    )

    # 7) 拼接并返回
    sorted_df = pd.concat([obs_df, lat_df])
    return sorted_df
