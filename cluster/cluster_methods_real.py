...
def cluster_main1(data_obs, model_es, marginal_prob_01, true_labels, num_cluster_new, num_runs, random_seed):
    ...
    # # 无需取消注释
    # filename = f'../result/frog/cluster/2_4_1%_c10_n1_{seed}.csv'
    # # 保存结果到 CSV 文件
    # result_df.to_csv(filename, index=False, encoding='utf-8-sig')


    folder = f'../result/frog/cluster'
    filename = f'{folder}/1.9_2.3_100%_c10_n1_{seed}.csv'
    # 创建文件夹（如果不存在）
...
for seed in range(42,52):
    random_seed = seed
    data_obs = pd.read_csv(f"../result/frog/frog_data_obs_{seed}_100%.csv", index_col='id')
    model_es = pd.read_csv(f"../result/frog/frog_ModelEs_renum.csv")
    marginal_prob = pd.read_csv(f"../result/frog/frog_marginal_prob_{seed}_100%.csv", index_col='id')
    true_labels_df = pd.read_csv(f"../result/frog/frog_true_labels_{seed}_100%.csv", index_col='id')
